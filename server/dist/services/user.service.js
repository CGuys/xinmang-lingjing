...
class UserService {
     ...
     * 3. 否则拦截并抛出 403 错误
     */
    static async deductEnergy(userId) {
        const today = (0, date_1.getCSTTodayString)();
        const user = await prisma_1.prisma.user.findUnique({
            where: { id: userId }
        });
        ...
        if (user.is_blacklisted) {
            throw new error_middleware_1.AppError('账号处于风控受限状态', 403, 'USER_BANNED');
        }
        // 优先消耗今日免费点
        ...
        // 均无能量，拦截
        throw new error_middleware_1.AppError('今日灵感已消耗完毕，邀请好友或观看短片即可补充灵感点', 403, 'ENERGY_EXHAUSTED');
    }
    /**
     * 增加奖励点数 (裂变或广告观看完播奖励)
     */
    static async addBonusEnergy(userId, amount = 1) {
        const updated = await prisma_1.prisma.user.update({
            where: { id: userId },
