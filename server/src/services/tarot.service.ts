...
let cardDeckCache: TarotCard[] = [];

function getCardDeck(): TarotCard[] {
  if (cardDeckCache.length > 0) return cardDeckCache;
...
export class TarotService {
   ...
   * 获取全部卡牌或按分类筛选
   */
  static getAllCards(category?: string) {
    const deck = getCardDeck();
    if (!category || category === 'all') return deck;
