...
async function runTests() {
    ...
    await new Promise((resolve) => {
        server = app_1.app.listen(TEST_PORT, () => {
    ...
    try {
        ...
        // 3. 静态图片资源加载测试
        const imgRes = await axios_1.default.get(`${BASE_URL}/assets/tarot/cards/00.jpg`, { responseType: 'arraybuffer' });
        ...
        // 4. 用户 Alice 微信静默登录
        const aliceLogin = await axios_1.default.post(`${BASE_URL}/api/v1/auth/wx-login`, {
        ...
        const aliceProfile = await axios_1.default.get(`${BASE_URL}/api/v1/user/profile`, { headers: aliceAuthHeader });
        ...
        const drawRes1 = await axios_1.default.post(`${BASE_URL}/api/v1/tarot/draw`, { question: '面对新挑战，我应当如何调整心理状态？' }, { headers: aliceAuthHeader });
        ...
        try {
            await axios_1.default.post(`${BASE_URL}/api/v1/tarot/draw`, { question: '再抽一次' }, { headers: aliceAuthHeader });
        ...
        const bobLogin = await axios_1.default.post(`${BASE_URL}/api/v1/auth/wx-login`, {
        ...
        try {
            await axios_1.default.post(`${BASE_URL}/api/v1/share/accept`, { inviter_id: bobId }, { headers: bobAuthHeader });
            assert(false, '自邀未被拦截');
        }
        catch (err) {
        ...
        const shareAcceptRes = await axios_1.default.post(`${BASE_URL}/api/v1/share/accept`, { inviter_id: aliceId }, { headers: bobAuthHeader });
        ...
        const repeatAcceptRes = await axios_1.default.post(`${BASE_URL}/api/v1/share/accept`, { inviter_id: aliceId }, { headers: bobAuthHeader });
        ...
        const drawRes2 = await axios_1.default.post(`${BASE_URL}/api/v1/tarot/draw`, { question: '感谢朋友赋能，求问今日灵感' }, { headers: aliceAuthHeader });
        ...
        await axios_1.default.post(`${BASE_URL}/api/v1/ad/reward-callback`, {
            trans_id: `trans_mock_test_sens_${runId}`,
            user_id: aliceId
        });
        try {
            await axios_1.default.post(`${BASE_URL}/api/v1/tarot/draw`, { question: '我想找大师帮我改运看相' }, { headers: aliceAuthHeader });
            assert(false, '敏感词未拦截');
        }
        catch (err) {
        ...
        const streamRes = await axios_1.default.get(`${BASE_URL}/api/v1/tarot/stream/${readingId}`, {
            headers: {
                ...aliceAuthHeader,
        ...
        // 12. 验证用户历史记录
        const historyRes = await axios_1.default.get(`${BASE_URL}/api/v1/tarot/history`, { headers: aliceAuthHeader });
        assert(historyRes.data.data.items.length >= 2, '存在至少 2 条历史翻牌');
        const streamedItem = historyRes.data.data.items.find((it) => it.id === readingId);
        ...
        // 13. 管理员登录
        const adminLoginRes = await axios_1.default.post(`${BASE_URL}/api/v1/admin/login`, {
            username: 'admin',
            password: 'admin123'
        });
        ...
        const dashRes = await axios_1.default.get(`${BASE_URL}/api/v1/admin/dashboard`, { headers: adminAuthHeader });
        assert(dashRes.data.data.totalUsers >= 2, '统计用户总数准确');
        assert(dashRes.data.data.todayDrawCount >= 2, '统计今日抽牌次数准确');
        // 动态策略配置更新与热生效测试
        await axios_1.default.put(`${BASE_URL}/api/v1/admin/strategy`, { is_in_review: true, share_reward_limit: 5 }, { headers: adminAuthHeader });
        const updatedStrategy = await axios_1.default.get(`${BASE_URL}/api/v1/admin/strategy`, { headers: adminAuthHeader });
        ...
        // 卡牌库查询测试
        const cardsRes = await axios_1.default.get(`${BASE_URL}/api/v1/admin/cards?category=major`, { headers: adminAuthHeader });
        assert(cardsRes.data.data.cards.length === 22, '大阿卡纳卡牌数为 22 张');
        // 用户管理列表与人工能量调整测试
        const usersRes = await axios_1.default.get(`${BASE_URL}/api/v1/admin/users`, { headers: adminAuthHeader });
        assert(usersRes.data.data.items.length >= 2, '用户管理查询正常');
        await axios_1.default.post(`${BASE_URL}/api/v1/admin/users/${aliceId}/energy`, { amount: 10 }, { headers: adminAuthHeader });
        const checkAlice = await axios_1.default.get(`${BASE_URL}/api/v1/user/profile`, { headers: aliceAuthHeader });
